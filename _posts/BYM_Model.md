...
In this section we specify the parameters that we will keep track of. Note that the parameters that are to be dynamically estimated is in transformed parameters section. In addition the parameter $\lambda$ in \eqref{eq:lambda} is referred to as alpha in this section.
